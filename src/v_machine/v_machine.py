...
def load_video(path, idx, mtd_dict, lru_list, loading_indices, max_num_mtd=4) -> mtd_video.MTDVideo:
    ...
    fp.close()
    load_duration = time.time() - start_time  # Berechnung der verstrichenen Zeit
    print(f"File {path} loaded in {load_duration} seconds.")  # Ausgabe der Ladezeit
    mtd_dict[idx] = mtd
...
class GUI(QWidget):
    ...
    def __init__(
        ...
        # Initialisiere den Timer-Status auf False
        """
        ...
        self.timer = QTimer()
        # Verbinde den Timeout-Event des Timers mit der load_next_video-Methode
    ...
    #Für den Timer
    def load_next_video_timer(self):
        # Diese Methode wird vom Timer aufgerufen und lädt das nächste Video
        self.right()
    ...
    def update(self, dim_1_dir=0, dim_0_dir=0):
        ...
        #if dim_0_dir == 1:
        #    target_img_idx[0] = (self.current_img_idx[0] + self.speed_multiplier) % self.mtd_shape[0]
            ...
            if dim_1_dir == 1:
                if not self.current_img_idx[1] == self.mtd_shape[1] - 1:
                    ...
                    if keyframe is not None:
                        ...
                        # Setze ein Standard-Differenzbild, wenn keines gefunden wird
                        if diff_image is None:
                            diff_image = np.zeros_like(self.current_frame)
                        self.current_frame = (
                            np.clip(
...
class SoundMonitor:
    ...
    def callback(self, indata, frames, ctime, status):
        # FFT (Fast Fourier Transform) der Audiodaten
        fft_result = scipy.fftpack.fft(indata[:,0])
        freqs = scipy.fftpack.fftfreq(len(fft_result), d=1.0/self.gui.sound_monitor.stream.samplerate)

    # Frequenzbereiche definieren
        low_freq_threshold = 200  # Bässe (in Hz)

    # Frequenzbereich für positive Frequenzen
        positive_freqs = freqs > 0

    # Berechnung der Energie im Bassbereich
        low_freq_power = np.sum(np.abs(fft_result[(positive_freqs) & (freqs < low_freq_threshold)]))

    # Schwellenwert für Bässe
        low_threshold = 0.01

    # Prüfen, ob Bass über dem Schwellenwert liegt
        bass_active = low_freq_power > low_threshold

    # Logik für relative Lautstärke
        amplitude = np.mean(abs(indata))
        n_average = np.mean(self.last_n)
        dim_1_dir = 0
        if amplitude < (n_average * (self.gui.threshold - 0.01)) or amplitude < 0.001:
    ...
    # Zufällige Bewegung basierend auf Amplitude
        dim_0_dir = 1 if 10 * amplitude > random.random() * self.gui.threshold else 0

    # Geschwindigkeitsanpassung basierend auf dominanten Bässen
        if bass_active:  # Wenn Bässe aktiv sind
            self.gui.update_speed_multiplier(10)
        else:
            self.gui.update_speed_multiplier(1)

    # Senden der Steuersignale an die GUI
        self.signal.emit(dim_1_dir, dim_0_dir)
        self.last_n.append(amplitude)
        self.last_n = self.last_n[-200:]
